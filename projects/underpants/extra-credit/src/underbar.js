(function() {
  ...
  _.filter = function(collection, test) {
    let result = [];
    for (let i=0; i < collection.length; i++) {
      if (typeof test(collection[i], i, collection) === 'boolean') {
  ...
  _.uniq = function(array, isSorted, iterator) {
    if (iterator === undefined) {
      ...
      let indexes = [];
      for (let i=0; i < array.length; i++) {
          if (!indexes.includes(array.indexOf(array[i]))) {
  ...
  _.reduce = function(collection, iterator, accumulator) {
    ...
    if (Array.isArray(collection)) {
      for (let i=0; i < collection.length; i++) {
        if (i === 0) {
          if (accumulator !== undefined) {
            prev = iterator(accumulator, collection[0], 0);
          } else {
            prev = iterator(collection[0], collection[1], 1);
            i += 1;
          }
        } else if (i < collection.length - 1) {
            prev = iterator(prev, collection[i], i);
        } else if (i === collection.length - 1) {
          result = iterator(prev, collection[i], i);
        }
      }
      return result;
    } else if (typeof collection === "object") {
      let objToArray = [];
      for (let key in collection) {
        objToArray.push(collection[key]);
      }
      return _.reduce(objToArray, iterator, accumulator);
    }
  };
